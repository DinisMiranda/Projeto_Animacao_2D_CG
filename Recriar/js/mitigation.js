...
let totalMitigation = 0;        // Mitigação total (máximo: 0.4 = 40%)

/**
 * Recalcula a mitigação total baseada nos painéis solares e mitigação extra
 * Cada painel solar colocado corretamente adiciona 0.1 (10%)
 * O autocarro na estrada adiciona 0.15 (15%)
 * O máximo total é 0.4 (40% de redução de poluição)
 */
function recalcMitigation() {
    ...
    // Calcula a mitigação total somando painéis + mitigação extra (autocarro, sorriso)
    // Math.min() garante que nunca ultrapasse 0.4 (40% máximo)
