...
function animationLoop(ts) {
    ...
    // Atualiza o arraste com a mão (apenas se o módulo handDetection.js estiver carregado)
    if (typeof updateHandDragging !== 'undefined') {
        // Atualiza o arraste de objetos com a mão
        updateHandDragging();
    }
...
function drawScene() {
    ...
    // === DESENHA INDICADOR DA MÃO (DEBUG) ===
    // Desenha indicador visual da posição da mão quando detecção está ativa
    if (typeof drawHandIndicator !== 'undefined') {
        drawHandIndicator();
    }
}
...
function mainLoop(ts) {
    ...
    // Sempre atualiza o arraste com a mão (fora do if para funcionar mesmo durante animações)
    if (typeof updateHandDragging !== 'undefined') {
        updateHandDragging();
    }
